...
export default function Category() {
  ...
  const onSubmit = (id) => {
    ...
    fetch(`https://lib.evamp.in/${id ? `updateCategory/${id}` : "addCategory"}`, {
      method: id ? 'PUT' : 'POST',
      headers: {
  ...
  const handleAssessmentSubmit = (id) => {
    const loading = toast.loading('Please wait...!');
    fetch(`https://lib.evamp.in/addCategoryAssessment/${comInfo.company_id}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/Json'
      },
  ...
  const handleDelete = (id, status) => {
    const loading = toast.loading('Please wait...!');
    fetch(`https://lib.evamp.in/${status ? "deleteCategoryAssessment" : "deleteCategory"}/${id}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/Json'
      }
  ...
  useEffect(() => {
    if (comInfo?.company_id) {
      fetch(`https://lib.evamp.in/getCategoryAssessment/${comInfo?.company_id}`)
        .then(res => res.json())
