...
export default function Planning() {
  ...
  const onSubmit = () => {
    ...
    fetch(
      `http://localhost:3333/${staterInfo ? 'update' : 'add'}GettingInfo/${
        staterInfo ? staterInfo.id : comInfo?.company_id
      }`,
      {
      ...
      .then((data) => {
        ...
        if (!data.error) {
          return swal(
            `Getting Info ${staterInfo ? 'updated' : 'added'}`,
            `GettingInfo has been ${staterInfo ? 'updated' : 'added'} successful.`,
            'success'
          );
        }
      ...
      <Card sx={{ p: 3 }}>
        <Grid container spacing={2}>
          ...
          <Drawer anchor="right" open={state} onClose={() => setState(false)}>
            <Stack sx={{ width: 450, mt: 2 }} alignItems="center" justifyContent="center" mb={3}>
              <img
                style={{ width: '150px', height: '150px' }}
                src={
                  preview !== null
                    ? preview
                    : 'https://www.prestophoto.com/storage/static/landing/pdf-book-printing/pdf-icon.png'
                }
                alt="logo"
              />
              ...
              />
              <Button sx={{ mt: 2 }} onClick={onSubmit} color="success" variant="outlined">
          ...
          {/* <Grid item xs={6}>
            {staterInfo?.docs ? (
              ...
              <Typography variant="h6">
                Your employee experience is a vital to shaping organizational culture and
                performance - today and in the future! <br /> <br />
                This process will enable you to anonymously provide your insight to help strengthen
                those areas that are enabling you and identity and prepare for changes to grow in
                the future.
                <br /> <br />
                Your customers are counting on you to be the best you can be! <br /> <br />
                Please take the next few minutes to provide your honest input. The data will be
                compiled and reported on in such a way that individual responses will not be
                identifiable.
              </Typography>
            )}
          </Grid> */}
